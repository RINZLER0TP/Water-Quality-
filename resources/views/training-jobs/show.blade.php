    ...
    <x-slot name="header">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div class="max-w-2xl space-y-3">
                ...
                <div class="space-y-2">
                    <h2 class="text-3xl font-semibold tracking-tight text-slate-950 leading-tight">Entrenamiento #{{ $trainingJob->id }}</h2>
                    <p class="max-w-xl text-sm leading-6 text-slate-500">Resultados reales del pipeline ejecutado sobre el dataset asociado.</p>
                </div>
            </div>
    ...
    <div class="py-10">
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_16px_60px_rgba(15,23,42,0.06)]">
                    <p class="text-sm font-medium text-slate-500">Estado</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ $trainingJob->status->label() }}</p>
                </div>
                <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_16px_60px_rgba(15,23,42,0.06)]">
                    <p class="text-sm font-medium text-slate-500">Accuracy</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ isset($trainingJob->metrics['accuracy']) ? number_format((float) $trainingJob->metrics['accuracy'] * 100, 2) . '%' : 'n/a' }}</p>
                </div>
                <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_16px_60px_rgba(15,23,42,0.06)]">
                    <p class="text-sm font-medium text-slate-500">F1-score</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ isset($trainingJob->metrics['f1_score']) ? number_format((float) $trainingJob->metrics['f1_score'] * 100, 2) . '%' : 'n/a' }}</p>
                </div>
                <div class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_16px_60px_rgba(15,23,42,0.06)]">
                    <p class="text-sm font-medium text-slate-500">Tiempo</p>
                    <p class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ $trainingJob->training_time_ms ? number_format($trainingJob->training_time_ms / 1000, 2).' s' : 'n/a' }}</p>
                </div>
            </section>
                ...
                <div class="space-y-6 min-w-0">
                    <div class="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="rounded-full bg-cyan-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.16em] text-cyan-700">Resumen</span>
                        </div>
                        <div class="mt-5 grid gap-4 sm:grid-cols-2">
                            <div class="rounded-2xl bg-slate-50 px-4 py-4">
                                <p class="text-xs uppercase tracking-wide text-slate-400">Dataset</p>
                                <p class="mt-1 break-words font-semibold text-slate-950">{{ $trainingJob->trainingConfiguration?->dataset?->name ?? 'n/a' }}</p>
                            </div>
                            <div class="rounded-2xl bg-slate-50 px-4 py-4">
                    ...
                    <div class="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="text-xs text-slate-400">Vista tabular</span>
                        </div>
                        <div class="mt-4 overflow-x-auto rounded-[24px] border border-slate-200 bg-slate-50">
                ...
                <div class="space-y-6 min-w-0">
                    <div class="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="text-xs text-slate-400">Cross-validation</span>
                        </div>
                        <div class="mt-5 grid gap-3">
                            @foreach (['accuracy' => 'Accuracy', 'precision' => 'Precision', 'recall' => 'Recall', 'f1_score' => 'F1-score', 'training_time_ms' => 'Tiempo entrenamiento (ms)'] as $key => $label)
                                <div class="grid grid-cols-[1fr_auto] items-center gap-4 rounded-2xl bg-slate-50 px-4 py-3 text-sm">
                    ...
                    <div class="rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)]">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span class="text-xs text-slate-400">Salida del engine</span>
                        </div>
                        <pre class="mt-4 max-h-[420px] overflow-auto rounded-[24px] bg-slate-950 p-4 text-xs leading-6 text-slate-100 whitespace-pre-wrap break-words">{{ $trainingJob->execution_log ?? $trainingJob->error_message ?? 'Sin logs disponibles.' }}</pre>
                    </div>
                </div>
            </section>
        </div>
